...
const ServiceAccess = ({
  ...
  function portFromService(service: ServiceData) {
    ...
    let port = service.options.find((option) =>
      option.nixName?.includes('port')
    )?.value
    if (port) return port
    // Otherwise get the default port
    return serviceByName(service.nixName)?.options.find((option) =>
      option.nixName?.includes('port')
    )?.value
  }
  ...
  const sshKeyOption = userData?.options?.find(
  ...
  return (
    <>
      ...
      <ul className="flex flex-wrap">
        {services.map((service) => (
          <li key={service.nixName} className="mb-2 mr-2">
            <Button>
              <a
                href={`http://${ip}:${portFromService(service)}`}
                target="_blank"
      ...
      <p>
        {' '}
        The following openssh public keys are whitelisted on the xnode user:{' '}
      </p>
      {sshKey && (
        <div className="auto block w-full overflow-auto border p-2">
          {' '}
          <p className="text-nowrap"> {sshKey} </p>{' '}
        </div>
      )}
      <br></br>
      ...
      <Dialog>
        ...
        <DialogContent style={{ maxHeight: '400px', overflowY: 'auto' }}>
          <DialogHeader>
            ...
            <DialogDescription>
              {' '}
              Enter whitelisted ssh keys separated by newlines.{' '}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <DialogClose onClick={() => updatedUserData(sshUserData(sshKey))}>
              {' '}
              Save{' '}
            </DialogClose>
          </DialogFooter>
