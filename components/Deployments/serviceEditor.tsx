        ...
        if (option.value?.includes(`"`)) {
          ...
          return (
            <input
              type="text"
              defaultValue={''}
              placeholder={optionValueWithoutQuotes}
              onChange={(e) =>
                updateFunc(
                  e.target.value,
                  serviceIndex,
    ...
    <>
      <tr>
        <td className="border border-gray-300 px-2 py-1">
          {option.name ? option.name : option.nixName}
        </td>
          ...
          <td colSpan={3}>
            <details>
              ...
              <table className="options-table w-full border-collapse border border-gray-300">
                ...
                <tbody>
                  {option.options.map(
                    (suboption: ServiceOption, suboptionIndex: number) => (
                      <OptionRow
                        key={suboptionIndex}
                        option={suboption}
...
const ServiceEditor = ({
  ...
  const updateFunc = (
    ...
    if (parentOptionIndex === null) {
      ...
      newServices[serviceIndex].options[parentOptionIndex].options[
        optionIndex
      ].value = newValue
    } else {
          ...
          {services.length ? (
            services.map((service, index) => (
              <Dialog key={service.nixName}>
                <TableRow>
                  <TableCell>
                    {service.name ? service.name : service.nixName}
                  </TableCell>
                  <TableCell className="max-w-48">{service.desc}</TableCell>
                  ...
                  <TableCell>
                    <Button
                      onClick={() => {
                        resetServiceToDefault(service.nixName)
                      }}
                      className="inline-flex h-10 min-w-24 items-center justify-center whitespace-nowrap rounded-md border border-primary bg-primary/95 px-4 text-sm font-semibold text-white transition-colors hover:bg-primary focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50"
                    >
                      Reset to default
                    </Button>
                  </TableCell>
